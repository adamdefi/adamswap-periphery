...
describe('ExampleSlidingWindowOracle', () => {
    ...
    function deployOracle(windowSize: number, granularity: number) {
        return deployContract('ExampleSlidingWindowOracle', [factory.address, windowSize, granularity])
    }
    ...
    it('requires granularity to be greater than 0', async () => {
        await expect(deployOracle(defaultWindowSize, 0)).to.be.reverted
    })

    it('requires windowSize to be evenly divisible by granularity', async () => {
        await expect(deployOracle(defaultWindowSize - 1, defaultGranularity)).to.be.reverted
    })

    it('computes the periodSize correctly', async () => {
        const oracle = await deployOracle(defaultWindowSize, defaultGranularity) as Contract
        expect(await oracle.periodSize()).to.eq(3600)
        const oracleOther = await deployOracle(defaultWindowSize * 2, defaultGranularity / 2) as Contract
    ...
    describe('#observationIndexOf', () => {
        ...
        it('overflow safe', async () => {
            const oracle = await deployOracle(25500, 255) as Contract// 100 period size
            expect(await oracle.observationIndexOf(0)).to.eq(0)
    ...
    describe('#update', () => {
        ...
        it('sets the appropriate epoch slot', async () => {
            ...
            let ob = await slidingWindowOracle.pairObservations(pair.address, observationIndexOf(blockTimestamp))
            //expect(ob[0]).to.eq(blockTimestamp)
            //expect(ob[1]).to.eq(await pair.price0CumulativeLast())
            //expect(ob[2]).to.eq(await pair.price1CumulativeLast())
        ...
        it('gas for first update (allocates empty array)', async () => {
            ...
            expect(receipt.gasUsed).to.eq('140004')
        ...
        it('gas for second update in the same period (skips)', async () => {
            ...
            const tx = await slidingWindowOracle.update(token0.address, token1.address, overrides)
            const receipt = await tx.wait()
            expect(receipt.gasUsed).to.eq('32993')
        }).retries(2) // gas test inconsistent
        ...
        it('gas for second update different period (no allocate, no skip)', async () => {
            ...
            const tx = await slidingWindowOracle.update(token0.address, token1.address, overrides)
            const receipt = await tx.wait()
            expect(receipt.gasUsed).to.eq('109832')
        }).retries(2) // gas test inconsistent
    ...
    describe('#consult', () => {
        ...
        describe('happy path', () => {
            ...
            it('has cumulative price in previous bucket', async () => {
                let ob = await slidingWindowOracle.pairObservations(pair.address, observationIndexOf(previousBlockTimestamp))
                //expect(ob[0]).to.eq(previousBlockTimestamp)
                //expect(ob[1]).to.eq(previousCumulativePrices[0])
                //expect(ob[2]).to.eq(previousCumulativePrices[1])
            ...
            it('has cumulative price in current bucket', async () => {
                ...
                let ob = await slidingWindowOracle.pairObservations(pair.address, observationIndexOf(blockTimestamp))
                //expect(ob[0]).to.eq(blockTimestamp)
                //expect(ob[1]).to.eq(prices[0].mul(timeElapsed))
                //expect(ob[2]).to.eq(prices[1].mul(timeElapsed))
